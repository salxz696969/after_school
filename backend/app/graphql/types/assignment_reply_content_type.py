...
class AssignmentReplyContentType:
    ...
    def assignment_reply(self, info: strawberry.Info[Context]) -> Annotated[
        ...
        return info.context.assignment_reply_content_loader.assignment_reply_loader.load((self.id, tuple(fields)))  # type: ignore
    ...
    def medias(self, info: strawberry.Info[Context]) -> List[
        ...
        fields = extract_fields(info)
        return info.context.assignment_reply_content_loader.media_loader.load((self.id, tuple(fields)))  # type: ignore
